...
# ============================================================
# Step 1: Build fceumm as a static library (.a)
# ============================================================
echo "--- Step 1: Building fceumm static library ---"

cd "$ROOT_DIR"
make -f Makefile.libretro clean platform=libnx 2>/dev/null || true
make -f Makefile.libretro -j$(nproc) platform=libnx

STATIC_LIB="$ROOT_DIR/fceumm_libretro_libnx.a"
...
COMMON_FLAGS="$COMMON_FLAGS -I$ROOT_DIR/src/drivers/libretro/libretro-common/include"
...
# ============================================================
echo "--- Step 3: Linking fceumm_tico.elf ---"

ELF_OUTPUT="$BUILD_DIR/fceumm_tico.elf"

LINK_FLAGS="-specs=$LIBNX/switch.specs -march=armv8-a+crc+crypto -mtune=cortex-a57 -mtp=soft -fPIE"
LINK_FLAGS="$LINK_FLAGS -Wl,--gc-sections -Wl,--allow-multiple-definition -Wl,-Map=$BUILD_DIR/fceumm_tico.map"
...
NRO_OUTPUT="$BUILD_DIR/tico-fceumm.nro"
ELF2NRO="$DEVKITPRO/tools/bin/elf2nro"
NACPTOOL="$DEVKITPRO/tools/bin/nacptool"

# Create NACP
NACP_FILE="$BUILD_DIR/fceumm.nacp"
$NACPTOOL --create "tico FCEUMM" "ticoverse.com" "1.0.1" "$NACP_FILE"
...
if [ -f "$NRO_OUTPUT" ]; then
    ...
    echo "Error: tico-fceumm.nro not found"
    exit 1
fi
