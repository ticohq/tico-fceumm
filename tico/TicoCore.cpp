/// @file TicoCore.cpp
/// @brief Simplified libretro frontend for fceumm with tico overlay
/// HW render via GLSM, save data uses native formats with .srm fallback
...
// Forward declarations for fceumm core functions (C linkage)
extern "C"
{
...
static uint32_t RC_CCONV RAReadMemory(uint32_t address, uint8_t* buffer, uint32_t num_bytes, rc_client_t* client)
{
    ...
    if (wram && address < wram_size && address + num_bytes <= wram_size) {
        memcpy(buffer, wram + address, num_bytes);
        return num_bytes;
    }
    
    // NES SRAM (usually mapped at 0x6000 in RA addressing)
    ...
    if (sram && address >= 0x6000 && (address - 0x6000) + num_bytes <= sram_size) {
        memcpy(buffer, sram + (address - 0x6000), num_bytes);
        return num_bytes;
    }
...
bool TicoCore::Init()
{
    ...
    // Ensure system/fceumm directories exist
    struct stat st = {0};
    if (stat(m_systemDir.c_str(), &st) == -1) {
        mkdir(m_systemDir.c_str(), 0777);
    }
    std::string coreSysDir = m_systemDir + "fceumm/";
    if (stat(coreSysDir.c_str(), &st) == -1) {
        mkdir(coreSysDir.c_str(), 0777);
    }
...
void TicoCore::RunFrame()
{
    ...
    else
    {
        ...
        if (m_rewindFrameCounter >= 2) // Save state every 2 frames for smoother rewind
        {
            ...
            if (size > 0 && size < 1024 * 1024 * 10) // Sanity check to not allocate gigabytes (FCEUMM states are small)
            {
...
bool TicoCore::HandleEnvironment(unsigned cmd, void *data)
{
    ...
    //==================================================================
    // Additional environment commands required by fceumm
    //==================================================================
                                  ...
                                  unsigned height, size_t pitch)
{
    ...
    if (!m_hwRender && data)
    {
        ...
        // FCEUMM uses 0RGB1555 or XRGB8888 where the alpha channel is 0. 
...
#ifdef __SWITCH__
    configPath = "sdmc:/tico/config/cores/fceumm.jsonc";
#else
    configPath = "tico/config/cores/fceumm.jsonc";
#endif
