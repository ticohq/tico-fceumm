/// @file TicoMain.cpp
/// @brief Entry point for tico-integrated fceumm NRO
...
bool InitWindow()
{
    ...
    g_window = SDL_CreateWindow("fceumm",
...
void Render()
{
    ...
    else
    {
        ...
        if (tex != 0)
        {
            g_simpleRenderer.Render(tex, w, h, ar);
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    {
        if (argv[i] && strcmp(argv[i], "ticoSetup") == 0)
        {
            g_useTico = true;
            LOG_INFO("HOME", "ticoSetup argument found. Starting in Tico Mode.");
            break;
        }
    }
    ...
    if (!g_useTico)
    {
        LOG_INFO("HOME", "No ticoSetup argument. Starting in Common Libretro Mode.");
    }
    ...
    {
        LOG_INFO("HOME", "Initializing Simple Renderer...");
        g_simpleRenderer.Init();
    }
