...
/// @brief Overlay UI for tico-integrated fceumm (no disc support)
...
void TicoOverlay::RenderGame(ImDrawList *dl, ImVec2 displaySize, unsigned int texture,
    ...
    if (texture == 0) return;
    static constexpr int NES_BASE_W = 256, NES_BASE_H = 240;
    float dstWidth = displaySize.x, dstHeight = displaySize.y, offsetX = 0, offsetY = 0;
    if (m_displayMode == FceummDisplayMode::Integer) {
        int scale;
        if (m_displaySize == FceummDisplaySize::Auto) {
            int scaleX = (int)displaySize.x / NES_BASE_W, scaleY = (int)displaySize.y / NES_BASE_H;
            scale = std::min(scaleX, scaleY); if (scale < 1) scale = 1;
        } else { scale = (int)m_displaySize - 3; if (scale < 1) scale = 1; }
        dstWidth = NES_BASE_W * scale; dstHeight = NES_BASE_H * scale;
        if (dstWidth > displaySize.x) dstWidth = displaySize.x;
        if (dstHeight > displaySize.y) dstHeight = displaySize.y;
    } else {
        switch (m_displaySize) {
        case FceummDisplaySize::Stretch: dstWidth = displaySize.x; dstHeight = displaySize.y; break;
        case FceummDisplaySize::_4_3: {
...
void TicoOverlay::RenderSettingsMenu(ImDrawList *dl, ImVec2 displaySize) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (i == 0) { label = tr("emulator_display_mode"); value = (m_displayMode == FceummDisplayMode::Integer) ? tr("emulator_integer") : tr("emulator_display"); }
        else if (i == 1) {
            label = tr("emulator_size");
            if (m_displayMode == FceummDisplayMode::Integer) {
                switch (m_displaySize) { case FceummDisplaySize::_1x: value="1x"; break; case FceummDisplaySize::_2x: value="2x"; break; default: value=tr("emulator_auto"); break; }
            } else {
                switch (m_displaySize) { case FceummDisplaySize::Stretch: value=tr("emulator_stretch"); break; case FceummDisplaySize::_4_3: value="4:3"; break; case FceummDisplaySize::_16_9: value="16:9"; break; default: value=tr("emulator_original"); break; }
            }
        } else if (i == 2) {
...
bool TicoOverlay::HandleInput(SDL_GameController *controller) {
    ...
    if (dirChanged && m_currentMenu == OverlayMenu::Settings) {
        if (m_settingsSelection == 0) {
            m_displayMode = (m_displayMode == FceummDisplayMode::Display) ? FceummDisplayMode::Integer : FceummDisplayMode::Display;
            m_displaySize = (m_displayMode == FceummDisplayMode::Integer) ? FceummDisplaySize::Auto : FceummDisplaySize::_4_3;
            ApplyScalingSettings(true);
        } else if (m_settingsSelection == 1) {
            if (m_displayMode == FceummDisplayMode::Integer) {
                int s = (int)m_displaySize + dir; if (s < 4) s = 6; if (s > 6) s = 4;
                m_displaySize = (FceummDisplaySize)s;
            } else {
    ...
    if (confirm && !m_confirmHeld && debounced) {
        ...
        if (m_currentMenu == OverlayMenu::QuickMenu) {
            ...
            if (m_settingsSelection == 0) {
                ...
                if (m_displayMode == FceummDisplayMode::Integer) { int s = (int)m_displaySize; s = (s >= 6) ? 4 : s+1; m_displaySize = (FceummDisplaySize)s; }
                else { int s = (int)m_displaySize; s = (s >= 3) ? 0 : s+1; m_displaySize = (FceummDisplaySize)s; }
                ApplyScalingSettings(true);
            } else if (m_settingsSelection == 2) {
...
#ifdef __SWITCH__
    std::string configPath = "sdmc:/tico/config/cores/fceumm.jsonc";
#else
    std::string configPath = "tico/config/cores/fceumm.jsonc";
#endif
    ...
    if (file.is_open()) {
        ...
        if (!j.is_discarded()) {
            if (j.contains("display_mode") && j["display_mode"].is_string()) {
                m_displayMode = (j["display_mode"].get<std::string>() == "Integer") ? FceummDisplayMode::Integer : FceummDisplayMode::Display;
            } else m_displayMode = FceummDisplayMode::Display;
            if (j.contains("display_size") && j["display_size"].is_string()) {
                std::string v = j["display_size"].get<std::string>();
                if (v=="Stretch") m_displaySize = FceummDisplaySize::Stretch;
                else if (v=="16:9") m_displaySize = FceummDisplaySize::_16_9;
                else if (v=="Original") m_displaySize = FceummDisplaySize::Original;
                else if (v=="1x") m_displaySize = FceummDisplaySize::_1x;
                else if (v=="2x") m_displaySize = FceummDisplaySize::_2x;
                else if (v=="Auto") m_displaySize = FceummDisplaySize::Auto;
                else m_displaySize = FceummDisplaySize::_4_3;
            } else m_displaySize = FceummDisplaySize::_4_3;
            if (j.contains("shader_type") && j["shader_type"].is_string()) {
                std::string v = j["shader_type"].get<std::string>();
                if (v=="LCD") m_shaderSelection = 1;
                else if (v=="xBRZ") m_shaderSelection = 2;
                else if (v=="Eagle") m_shaderSelection = 3;
                else if (v=="Dot") m_shaderSelection = 4;
                else if (v=="LcdGridV2") m_shaderSelection = 5;
                else m_shaderSelection = 0;
            } else m_shaderSelection = 0;
        } else { m_displayMode = FceummDisplayMode::Display; m_displaySize = FceummDisplaySize::_4_3; }
    } else { m_displayMode = FceummDisplayMode::Display; m_displaySize = FceummDisplaySize::_4_3; }
...
void TicoOverlay::SaveCoreSettings() {
    ...
    j["display_mode"] = (m_displayMode == FceummDisplayMode::Integer) ? "Integer" : "Display";
    const char *sizeStr = "4:3";
    switch (m_displaySize) {
    case FceummDisplaySize::Stretch: sizeStr="Stretch"; break; case FceummDisplaySize::_4_3: sizeStr="4:3"; break;
    case FceummDisplaySize::_16_9: sizeStr="16:9"; break; case FceummDisplaySize::Original: sizeStr="Original"; break;
    case FceummDisplaySize::_1x: sizeStr="1x"; break; case FceummDisplaySize::_2x: sizeStr="2x"; break;
    case FceummDisplaySize::Auto: sizeStr="Auto"; break; default: break;
    }
    j["display_size"] = sizeStr;
    const char *shaderStr = "None";
    switch (m_shaderSelection) {
    case 1: shaderStr = "LCD"; break;
    case 2: shaderStr = "xBRZ"; break;
    case 3: shaderStr = "Eagle"; break;
    case 4: shaderStr = "Dot"; break;
    case 5: shaderStr = "LcdGridV2"; break;
